<x-layout>
    ...
    <form method="GET" action="{{route('books.index')}}" class="mb-4 flex items-center space-x-2">
        <input type="text" name="title" placeholder="Search by Tilte..."
    ...
    <div class="filter-container mb-4 flex">
        ...
        @foreach ($filters as $key => $label)
            <a href="{{route('books.index', [...request()->except('page'), 'filter' => $key])}}"
  ...
  <div>{{$books->appends(request()->query())->links()}}</div>
</x-layout>
